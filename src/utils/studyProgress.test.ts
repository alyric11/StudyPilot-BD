...
const done: ChapterProgress = {
  readTextbook: true, watchedLectures: true, solvedExercises: true,
  solvedBoardQuestions: true, madeNotes: true, revisionCompleted: true,
};
const bangla = class12Subjects("Science").find((subject) => subject.id === "bangla_1")!;

test("chapter revisions cannot overflow section totals", () => {
  const saved = Object.fromEntries(bangla.chapters.slice(0, 5).map((chapter) => [chapter.id, done]));
...
test("a unit is revised only when every chapter in that unit is revised", () => {
  const saved = Object.fromEntries(bangla.chapters.filter((chapter) => chapter.section === "গদ্য").map((chapter) => [chapter.id, done]));
...
test("missing records count as incomplete and stale records do not inflate progress", () => {
  ...
  assert.equal(getStudyProgress(bangla, { stale: done }).percentage, 0);
  const all = Object.fromEntries(bangla.chapters.map((chapter) => [chapter.id, done]));
